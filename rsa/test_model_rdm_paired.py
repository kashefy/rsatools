...
from rsa.model_rdm_paired import ModelRDMPaired
...
class TestModelRDMPairedInput2DMatNPZ(TestModelRDMPairedInput2DMat):
    ...
    def helper_calc_model_rdm(self, flist):
        ...
        m = ModelRDMPaired(flist_npz)
        m.set_loader(loader)
        mrdm = m.apply(do_disable_tqdm=True)
        return mrdm


class TestModelRDMPairedInputTriuVec(TestModelRDMPairedInput2DMat):
    ...
    def setup(self):
        TestModelRDMPairedInput2DMat.setup(self)
        in_rdm1 = np.array([[0, 1, 2],
                            [1, 0, 0.5],
                            [2, 0.5, 0]])
...
class ModelRDMPairedScaled(ModelRDMPaired):
    def dissimilarity(self, fp_row, fp_col, idx):
        idx, dissimilarity = ModelRDMPaired.dissimilarity(self, fp_row, fp_col, idx)
        return idx, 100 * dissimilarity


class TestModelRDMCorrelation(TestModelRDMPairedInputTriuVec):
    ...
    def test_calc_model_rdm_values(self):
        ...
        m = ModelRDMPairedScaled(fp_list)
        mrdm = m.apply(do_disable_tqdm=True)
        mrdm = triu_off_diag_to_mat(mrdm)
        mrdm += mrdm.T
        for r, fp_r in enumerate(fp_list):
